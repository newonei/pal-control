...
1. 检查版本、世界、赛季、撤离和购买闸门。
2. 按 account 加 PostgreSQL advisory lock，避免同时购买/撤离导致快照竞争。
...
| 撤离报价后玩家移动物品 | commit 以 412 拒绝，无扣物、无入账 |
...
购买只依赖 PalDefender REST，可与依赖 RCON 的撤离使用独立闸门；但世界身份或数据库异常必须同时关闭两者。
