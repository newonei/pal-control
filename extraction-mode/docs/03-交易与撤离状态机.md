...
1. 检查 current content、世界、赛季和资源兑换闸门；至少一个内容定义区域必须在当前服务端时间开放。
...
2. 比较 quote 冻结的 `contentVersionId + contentHash` 与 current pointer；任何一项变化都以 `QUOTE_CONTENT_CHANGED` 拒绝，且不扣物。
3. 再读玩家身份与位置，必须仍在线且在原区域；区域在报价后关闭时只允许内容定义的 grace 内完成，越界后拒绝。
...
8. Control API 要求 Native hello 声明已批准的稳定 `inventory.consume` 能力、正确游戏/MOD/协议版本和 persistence 证据；当前仅有 `inventory.consume.experimental` 时在发送前关闭闸门。
