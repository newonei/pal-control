...
本目录是该玩法的权威设计基线。它把现有 Pal Control、PalDefender 和 Palworld 专用服务器能力组合成一个可实施的 MVP，并明确标出尚未实现的硬门槛。
...
- 商城只通过 Control API 调用本机 PalDefender；撤离首版通过 Control API 的本机白名单 RCON 适配器执行。浏览器绝不直连 PalDefender、RCON、Native Bridge 或官方 REST。
- 平台 `UserId` 是跨档身份；`PlayerUID` 只在一个世界内有效，不能用玩家名绑定账户。
- 商城与撤离均使用数据库账本、幂等键、Outbox 和可对账状态机；任何 `uncertain` 都禁止自动重发。
- MVP 撤离只结算 `Items`、`Food`、`DropSlot` 三类容器中的白名单物品；武器、护甲、关键物品和帕鲁暂不结算。
- **先扣除且通过 REST 背包回读证明物品已经消失，后增加数据库货币**。RCON 文本响应本身不是成功证据；结果不能证明时，撤离功能保持冻结并进入对账。
...
| [03-交易与撤离状态机](docs/03-交易与撤离状态机.md) | 商城发货、撤离扣物、幂等和故障恢复 |
...
| 能力 | 当前证据 | MVP 决策 |
| --- | --- | --- |
| 读取在线玩家、`UserId`、`PlayerUID`、位置 | PalDefender 玩家接口已接入 | 复用，用于身份映射与撤离区判断 |
| 读取六类背包 | PalDefender `GET items/{playerIdentifier}` 已接入 | 复用，生成商城审计快照与撤离报价 |
...
| 清空背包 | PalDefender RCON 提供 `/clearinv` | 破坏范围过大，不用于自动撤离，只允许开发服/人工应急 |
...
| 撤离报价与结算 | 已实现位置双采样、30 秒报价、白名单扣物、REST 后读和唯一入账 | 已用 5 皮革完成真实扣物与战备券入账；同键重放未重复入账 |
