...
- 每周一 `04:00`（`Asia/Shanghai`）换新世界；每日 `04:00` 刷新营业日内容、商城价格、日任务、动态开放区和纯经济世界事件。个人周限购只在新周档重置。
...
- 商品、回收白名单、兑换区、任务和轮换策略来自完整的不可变内容版本；当前授权目录完整时内置 10 个商品、51 个可售资源、2 个本地候选兑换区和 3 日/3 周可靠任务。每天在双点池中确定性开放 1 点，并为其声明风险等级和营业日开始 8 小时后的 3 小时限时热点；同时在“资源繁荣”收益事件与“战备补给”折扣事件中确定性选择 1 个。目录、地图、报价和 run 冻结同一版本/seed/window/multiplier 证据；事件不伪造击杀、掉落或采集。候选点 2 尚未通过真实 Palworld 坐标验收，旧 offer 不得跨 content hash 使用。
