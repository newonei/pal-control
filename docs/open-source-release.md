...
- [ ] 从本轮候选提交克隆全新副本并使用 npm 官方 registry 安装，重新运行两个前端构建、OpenAPI、管理台 21/21、玩家端 32/32、Chromium 15/15、统一 contract/integration 52/52 和 Control API Release；上一公开基线 `7568a4a` 的 44 项结果不能替代本轮验证；
- [x] 使用 Gitleaks CLI `8.30.1` 对本轮完整暂存树和全部 23 个可达历史提交重新扫描，均为 0 泄漏；固定错误曲线测试不再把 PEM 私钥块写入源码；
