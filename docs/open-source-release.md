...
- [x] 修正当前提交中明显过时的运行文档和机器专用路径；60 个 Markdown 本地链接、19 个 JSON 文件和 29 个 Markdown JSON 示例已复核；
- [x] 从候选代码提交 `7568a4a` 克隆全新副本并使用 npm 官方 registry 安装：两个前端构建、OpenAPI、管理台 21/21、玩家端 32/32、system Chrome 15/15、contract/integration 44/44 和 Control API Release（0 警告/0 错误）全部通过；
- [x] 对当前候选提交运行高置信度秘密扫描；Gitleaks CLI `8.30.1` 的 staged 与 19 个可达历史提交扫描均为 0 泄漏；
- [x] 人工审阅当前候选提交的完整暂存文件清单；221 个路径中无运行数据、存档、凭据、构建产物或第三方二进制，且无单文件超过 5 MiB。
