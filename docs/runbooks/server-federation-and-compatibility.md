...
- `v1.0.1.100619` / Steam build `24181105` + Native `0.3.0-dev.37-ro` 是历史 `quarantined` 组合。它曾受控建立 Bridge，但库存探针会把持久离线库存误当作 live inventory，现已 `superseded`，不得重新部署。矩阵中的 `bridgeAvailability=available` 只记录那次历史连接事实，不表示组合可用或受支持。
- 同一游戏/Steam build + Native `0.3.0-dev.38-ro` 是历史 `superseded/quarantined` 只读运行组合。888,320 字节、SHA-256 `012f84929448321196734b0bdc1f1b6a899a6f7a0aa87564d99b6e4c40b868aa` 的 DLL 曾受控建立 Bridge：9 项非玩家只读操作成功，3 项 live-player 探针因无人在线拒绝，0 项意外失败。矩阵中的 `bridgeAvailability=available` 只记录该历史会话，不表示当前版本可部署或任何门禁通过。
- 同一游戏/Steam build + Native `0.3.0-dev.39-ro` 是当前 `quarantined` 源码/制品候选。两个独立锁定构建均得到 893,440 字节、SHA-256 `c2dab9f9bfd3c47ac1a244139fb96ce1de6f598c4bce438ebddde96185063b34` 的字节一致 DLL，但尚未受控加载或运行固定探针套件，因此矩阵必须保持 `bridgeAvailability=unknown`。P0-04、在线玩家三项、PalDefender 组合、独立复核和写持久化均未通过。
...
dotnet run --project .\tools\compatibility-guard\PalControl.CompatibilityGuard.csproj `
  ...
  --combination pal-1.0.1.100619-build-24181105-source-dev39-ro
```
