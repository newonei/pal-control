...
| Native | 已为当前 `v1.0.1.100619` / build `24181105` 构建协议 1.1 `dev37-ro` 只读候选；注册 hook 前绑定精确 PalServer EXE 与 UE4SS，hello 上报 Native/UE4SS 摘要，固定 pipe/低权限 service SID，构建、hello、transport 与游戏线程四层禁止写入；两个全新锁定目录复现同一 DLL | dev37-ro 尚未部署/实服 ABI/schema probe；通过后还需独立写候选与真实保存/重启/重登，不能提升为 stable |
...
- [ ] 当前源码已完成 `v1.0.1.100619` / Steam build `24181105` 的协议 1.1 dev37-ro 只读候选、精确宿主 EXE 绑定和受控 C++ 构建；仍须在当前真实 PalServer 完成加载、ABI/schema 与三容器只读探针并由独立人员复核，才能固定 Palworld、UE4SS、Native 和 PalDefender 组合；旧 dev36 DLL 不得直接恢复加载。 <!-- gate:p0-04-current-native-probe -->
...
2026-07-17 本机复核：磁盘 Palworld 为 `v1.0.1.100619`（Steam build `24181105`），当前没有 PalServer/Control API/Caddy 进程。仓库已将 Native lock 更新为精确 PalServer EXE、UE4SS runtime、Native DLL 大小/SHA-256、固定 pipe/service SID、协议 `1.1` 和 `0.3.0-dev.37-ro`。受审 Cargo.lock、Cargo `--locked`、固定非浅克隆 Corrosion/IconFont 提交、UE4SS `c2ac246`、Unreal `b2e876d`、CMake `3.31.6`、Rust `1.97.0`、MSVC `19.44.35217` 在两个全新独立目录均得到 886,272 字节、SHA-256 `c91bee8f943b6c151a59c41ff0a51aebf36469b0c60f4201494cc5a3a416f8a7` 的只读候选；该身份已记录在 lock/兼容矩阵，但未部署。服务器磁盘仍保留旧 dev36，兼容守卫继续隔离它。编译成功不能证明运行时 ABI/schema，当前也没有受控测试玩家，因此本项和真实扣物持久化验收都继续未完成；严禁直接恢复旧 loader 或把 dev37-ro 手工改成 write/stable。
...
2. 先在当前真实 PalServer 部署 dev37-ro 完成 **P0-04** 的只读 ABI/schema 与三容器探针；独立复核通过后再构建显式写候选，由受控玩家完成“Native 扣物 → 保存 → 停服 → 重启 → 重登”验收并保存脱敏证据。
...
- Native consume 的稳定协议与持久化门禁：`packages/contracts/bridge/inventory.consume.md`；当前 dev37-ro 只读候选不发布任何 consume/write capability，旧 dev36 experimental 不能用于当前实服
