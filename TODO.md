...
> 审计基线：2026-07-17，本文随当前候选提交版本化；当前候选工作树已通过两个前端生产构建、管理台 21/21、玩家端 32/32、Chromium E2E 15/15、统一 contract/integration 52/52、Control API Release 0 警告/0 错误，以及两个独立外部源码副本的锁定 Native 可复现构建。推送后的 GitHub Actions secret scan 与 Windows build/test 也已对本轮代码候选全部通过。本文只描述当前仓库能够证明的能力，以及从“本机开发服原型”走到“可长期运营的周世界资源经济服”仍需完成的工作；22 个真实域名、管理员、玩家、实服、时间跨度或多节点验收门禁继续保持未勾选。设计文档、mock、单次手工演示或页面截图不等于功能完成。
...
52 个统一 contract/integration 脚本（15 contract + 37 integration）已经覆盖身份、商城、发货、兑换、管理员操作幂等键、版本化内容、内容投影原子性、可靠任务、永久币来源/消费上限、经济仿真、并发、故障边界、恢复、区域校准、周报归档、验收证据、短时非验收 soak、日志关联/脱敏审计、Windows 配置保留、生产部署/回切、SQLite 迁移一致性核对、换档客户端，以及团队投影与 RunStore 的 52 周/10,000 账户或 run 容量门禁；真实游戏证据仍主要是早期一次商城发货与一次 5 个皮革 RCON 历史联调，不能替代当前 Native 持久化、完整周档、24 小时 soak 和生产回调验收。
...
| Native | 当前 `v1.0.1.100619` / build `24181105` 使用协议 1.1 `dev39-ro` 只读源码/制品候选；精确绑定 PalServer EXE 与 UE4SS，两个全新锁定目录复现同一 893,440 字节、SHA-256 `c2dab9f9…063b34` 的 DLL，但尚未实服加载或运行固定探针套件。9 项成功、3 项无人在线拒绝、0 项意外失败属于已 superseded/quarantined 的 dev38-ro 历史运行证据 | 当前只证明 dev39-ro 制品可复现，Bridge 可用性仍为 unknown；仍缺受控实服固定套件、在线玩家的玩家/成长/三容器库存探针、PalDefender 组合固定、独立复核、显式写候选和真实扣物持久化，不能提升为 stable |
...
- [ ] 在正式 HTTPS 域名完成 Steam OpenID `realm/return_to`、Caddy 反代头、Secure/SameSite Cookie、回调日志跳过和 state/nonce 重放黑盒验收；本机回环 mock 不能替代生产回调证据。 <!-- gate:p0-02-production-steam-openid -->
...
- [ ] 当前源码已完成 `v1.0.1.100619` / Steam build `24181105` 的协议 1.1 dev39-ro 只读候选、精确宿主 EXE/UE4SS 绑定，以及 893,440 字节、SHA-256 `c2dab9f9bfd3c47ac1a244139fb96ce1de6f598c4bce438ebddde96185063b34` 的双独立字节一致构建；尚未实服加载或运行固定探针套件。必须先取得 dev39-ro 的受控运行证据，再由受控在线玩家完成玩家、成长、三容器库存探针并由独立人员复核，同时固定 Palworld、UE4SS、Native 与 PalDefender 组合。dev38-ro 的 9 成功/3 无人在线拒绝仅是历史证据，不能转记给 dev39-ro；旧 dev36、缺陷 dev37-ro 和已 superseded 的 dev38-ro 均不得恢复加载。 <!-- gate:p0-04-current-native-probe -->
- [ ] 完成“保存世界 → 停服 → 重启 → 重登”的持久化验证，只有通过后才把能力从 experimental 提升为 stable。 <!-- gate:p0-04-native-persistence -->
...
2026-07-17 本机复核：磁盘 Palworld 为 `v1.0.1.100619`（Steam build `24181105`）。仓库已将 Native lock 更新为精确 PalServer EXE、UE4SS runtime、Native DLL 大小/SHA-256、固定 pipe/service SID、协议 `1.1` 和 `0.3.0-dev.39-ro`。受审 Cargo.lock、Cargo `--locked`、固定非浅克隆 Corrosion/IconFont 提交、UE4SS `c2ac246`、Unreal `b2e876d`、CMake `3.31.6`、Rust `1.97.0`、MSVC `19.44.35217` 在两个全新独立目录均得到 893,440 字节、SHA-256 `c2dab9f9bfd3c47ac1a244139fb96ce1de6f598c4bce438ebddde96185063b34` 的字节一致 dev39-ro 只读制品。dev39-ro 目前尚未实服加载，固定 12 项套件也尚未执行，Bridge 可用性必须保持 unknown。最近一次受控真实 PalServer 运行属于 888,320 字节、SHA-256 `012f84929448321196734b0bdc1f1b6a899a6f7a0aa87564d99b6e4c40b868aa` 的 dev38-ro：hello 身份校验通过且 `writeEnabled=false`，9 项非玩家只读操作成功，玩家、成长和库存 live probe 因无人在线拒绝，0 项意外失败；官方保存与优雅关服只证明维护流程。dev38-ro 现已被 dev39-ro 源码候选取代并保持 `superseded/quarantined`，不得把其运行结果当作 dev39-ro 证据。旧 dev36 仅保留历史，dev37-ro 因把持久离线库存误判为 live inventory 同样禁止重新部署。当前仍没有 dev39-ro 受控运行、受控在线玩家、PalDefender 组合验收或独立复核，因此 P0-04 两个门禁继续未完成，严禁把任何候选手工改成 write/stable。
...
- [ ] 在固定真实 PalDefender 版本上连续完成 200 次逐物品结构化发货，并在 timeout、partial、进程重启和每个故障边界各重复至少 20 次，确认无重复发货、余额差或无证据退款。 <!-- gate:p0-06-paldefender-delivery-campaign -->
...
- [ ] 由至少两个不同的真实管理员主体完成权限隔离、撤权、MFA 与审计追溯实操，并由非实现者复核；自动化 principal 只能证明代码边界。 <!-- gate:p0-07-real-admin-rbac -->
...
- [ ] 使用脱敏真实备份分别完成一次世界恢复和一次经济恢复，记录 RPO/RTO、备份时间点后的待人工核对交易及复核人。 <!-- gate:p0-09-independent-real-restores -->
- [ ] 从已发布 N-1 安装包执行升级并故意触发迁移失败，验证冷快照回退；随后完成 staging 人工复核到生产目录切换，不得仅使用临时目录 harness 作为证据。 <!-- gate:p0-09-n-minus-one-migration-recovery -->
...
- [ ] 在固定版本真实 Palworld 上连续完成 3 次“旧世界 -> 备份验证 -> 新世界 -> 新赛季 -> 重新开闸”的完整周换档并保存脱敏证据。 <!-- gate:p0-10-three-live-rollovers -->
- [ ] 新赛季产生真实交易后，验证自动流程拒绝回滚旧世界，并由独立值班人员按手册完成处置。 <!-- gate:p0-10-post-transaction-rollback-refusal -->
...
- [ ] 由 5–10 名真实玩家在固定版本运行完整 7 天，期间不得跳过真实周换档，并归档匿名化运营记录。 <!-- gate:p0-11-seven-day-player-trial -->
- [ ] 最近 7 天确认无未解释余额差、重复发货、无依据入账或部分扣物；所有 `uncertain` 均有处置记录。 <!-- gate:p0-11-seven-day-economic-invariants -->
- [ ] 由未参与实现的值班人员独立完成订单/资源兑换 `uncertain` 对账和世界/经济备份恢复。 <!-- gate:p0-11-independent-reconciliation-restore -->
- [ ] 由不同于实现者的人复核全部 P0 证据、版本组合、日志脱敏和开闸结论。 <!-- gate:p0-11-independent-release-review -->
...
- [ ] 在真实 Palworld 坐标/开放边界中验收第 2 个资源兑换区；当前本地默认已有明确标注“待实服校准”的候选点 2 和双点自动化，但候选/mock 不能代替实测。 <!-- gate:p1-01-second-live-exchange-zone -->
...
- [ ] 在固定游戏版本核对模型外的真实制作配方、拆分/合成路径和玩家交易价格，记录未纳入 attested 影子图的转换及处置；运营模型不能代替游戏事实。 <!-- gate:p1-03-live-economy-facts -->
- [ ] 连续两个周档输出通胀、产销、热门商品和异常账户报告。 <!-- gate:p1-03-two-season-economy-reports -->
...
- [x] **条件当前未触发**：方案 A 的单服 5–10 人试运营继续使用单实例 SQLite；[ADR-0003](docs/architecture/decisions/0003-sqlite-capacity-and-postgresql-triggers.md) 已按 52 周/10,000 账户或 run 容量门禁、24 小时 soak、真实写锁/积压、RPO/RTO 与多实例/HA 需求冻结重新立项条件。若任一条件触发，必须实现 PostgreSQL 目标 reader、字段/类型/排序映射、transactional outbox 与 lease，并使用脱敏真实库逐账户/逐表核对；当前 SQLite 基线工具不能冒充 PostgreSQL 实迁移完成。若增加事件 hash 链，需另行版本化 canonical serialization。
- [ ] 由容量/架构负责人和独立复核人按 `p1-07-postgresql-capacity-migration` 签署当前 campaign 的容量决定：若条件未触发，提交 `capacity-slo-decision + sqlite-scope-review` 的受信 `not-applicable` 证据；若已触发，则必须完成真实 PostgreSQL 迁移、逐账户/逐表核对、transactional outbox 与 lease failover，不能只保留文字说明。 <!-- gate:p1-07-postgresql-capacity-migration -->
...
- [ ] 由非开发人员在全新 Windows VM 实操两次安装、升级和回滚，并使用脱敏真实备份完成独立恢复演练。 <!-- gate:p1-07-fresh-windows-vm-operations -->
- [ ] 完成 24 小时 soak test，确认内存、句柄、日志、队列和会话没有持续增长。 <!-- gate:p1-07-24-hour-soak -->

仓库内自动化已覆盖错误包 hash、重复安装、服务账户/外置配置、升级健康失败后的冷快照与旧版本恢复、同契约回滚保留升级后交易、跨契约回滚拒绝、真实 Control API 连续两次启动的 SQLite integrity/foreign keys/迁移幂等，以及迁移前后逐账户/逐行严格核对与篡改故障。当前拓扑按 ADR-0003 未触发 PostgreSQL 迁移；真实 SCM/ACL、Caddy TLS/Steam、空白机器、真实备份恢复和 24 小时 soak 仍属于外部验收，不能凭这些 harness 勾选整节完成。若重新立项 PostgreSQL，则实迁移和脱敏真实库核对同样是外部门禁。
...
- [ ] 按 `p2-team-economy-live-week` 由真实多人完成一个完整周世界，验证封禁投影、周换档、备份恢复和非奖励型目标的运营理解，并由独立复核人批准匿名化证据。 <!-- gate:p2-team-economy-live-week -->
...
- [ ] 按 `p2-analytics-live-week-sample` 采集真实连续一周的运营分析样本，逐分母核对权威来源、确认失败不计成功、少量群体隐藏且无玩家标识，并由独立复核人批准。 <!-- gate:p2-analytics-live-week-sample -->
- [x] 多 Palworld 服务器、跨服账户和兼容矩阵自动化：实现固定 allowlist 节点注册、Viewer 健康/矩阵 API、玩家 session 派生的 pair-bound HMAC subject、逐 peer v2 签名/限流/超时/响应上限/禁重定向、各节点权威 SQLite 只读账户/周档/双币聚合，以及 schema + canonical SHA-256 + CLI/PowerShell stable 门禁。3 节点 × 100 账户 harness 与真实 Control API 回环 HTTP smoke 覆盖 caller/target/key/subject 绑定、正文篡改、nonce 重放、过期时间、单 key/整 peer 吊销、signing/identity 双版本轮换、20 次只读请求、重启、IDOR、SSRF、oversize、timeout、redirect、节点掉线和矩阵漂移；玩家“我的服务器”页与真实 system Chrome 三节点用例还覆盖递归字段名去除横线/下划线/空白后的敏感身份 fail-closed、故障余额不冒充 0、精确 allowlist 门户链接和 Axe A/AA。跨服交易、转币、库存与拍卖不在范围。旧 `v1.0.0.100427`/dev36 仍是 experimental；dev37-ro 已因离线库存边界缺陷被取代并隔离；dev38-ro 保留历史 9 成功/3 无人在线拒绝的受控 Bridge 证据并标为 superseded/quarantined。当前 `v1.0.1.100619`/build `24181105` 的 dev39-ro 只有双独立可复现制品证据，Bridge 可用性 unknown，运行套件、在线玩家探针和独立复核未完成，因此同样是 quarantined。真实第二/第三服务器、生产 TLS/secret 轮换和各节点 stable 兼容验收仍是外部门禁，单服 Control API/worker 多实例继续由 P1-07 负责。
- [x] 团队经济 worker 只选择配置服务器唯一、已绑定、位于有效窗口的权威 Active season；零活动安全 no-op，多活动/无效活动 fail-closed，投影读事务再次核验活动周以阻断周切竞态。显式 `ProjectSeasonAsync` 仍可人工重建历史。容量 harness 使用 52 个历史周、10,000 账户、8 路 WAL writer 和 16 路团队写竞争，证明 51 个历史哨兵未被 worker 触碰、64 条并发事实只投影一次，并对投影/写锁耗时设硬上限。
- [x] `ExtractionRunStore` 已移除全部全历史 `CloneSnapshot` 写路径：SQLite CAS/credit 仍是提交点，提交前只克隆 changed runs 并预留字典容量，提交后无分配地原位替换对应槽；对外读取继续深 clone。容量 harness 使用 52 周、10,000 run 与 256 路并发结算，512 个普通批次均只写 1 行、256 次 credit 原子提交、9,744 个未变对象引用保持，双行选择批次故障整批不更新，单轮约 11 ms/2 MiB 分配。
- [x] 管理台除首屏仪表盘外的 12 个运营页面已按功能 `React.lazy + Suspense`；Vite manifest 后置门禁强制经济分析/运营等页面保持动态入口，并对 initial/最大 async/总 JS/CSS 分别设硬预算。当前 Release 主入口为 `226.50 kB`（较 `501.75 kB` 降 54.86%），最大 async `50.89 KiB`、总 JS `494.80 KiB`，21 项 Vitest、6 项预算故障测试和生产构建均通过。
- [x] 联邦入站协议已升级为 v2：`fed2_` subject 同时绑定版本化 `IdentityKeyId + caller + target + 平台身份`；每个请求再用逐 peer signing key 对 method/path、caller、target、signing/identity key id、目标 combination、timestamp、128-bit nonce 与正文 SHA-256 做 HMAC-SHA-256。接收端验证 profile body 与认证上下文组合一致、120 秒时钟窗、nonce 单次使用、调用方/本机 combination，并支持 identity/signing 双版本分阶段轮换、单 signing key 及整 peer 独立吊销；不同 peer 和入出站方向禁止复用 signing secret。单元、真实 Control API 回环 HTTP、OpenAPI、生产 secret 路径和部署回归均已覆盖；真实节点 TLS/轮换仍由下一项外部验收负责。
- [ ] 在真实第二、第三 Palworld 节点完成 TLS、稳定兼容矩阵、节点密钥轮换/吊销、节点掉线恢复与匿名化账户汇总验收。 <!-- gate:p2-three-node-federation -->
...
2. 先受控部署 dev39-ro 并执行固定 12 项只读套件；再由受控在线玩家补齐 **P0-04** 的玩家、成长和三容器库存探针，并在固定 PalDefender 组合下由独立人员复核。通过后再构建显式写候选，由受控玩家完成“Native 扣物 → 保存 → 停服 → 重启 → 重登”验收并保存脱敏证据；dev38-ro 仅保留历史证据，不得重新部署。
...
- Native consume 的稳定协议与持久化门禁：`packages/contracts/bridge/inventory.consume.md`；当前 dev39-ro 只读源码/制品候选不发布任何 consume/write capability且运行验证仍待完成，旧 dev36 experimental、缺陷 dev37-ro 与已 superseded 的 dev38-ro 都不能用于当前实服
...
- 团队投影活动周围栏与 52 周/10,000 账户并发容量证据：`services/control-api/Infrastructure/TeamEconomyProjectionWorker.cs`、`TeamEconomyProjection.cs`、`tests/team-economy-capacity/`、`tests/integration/team-economy-capacity-smoke.ps1` 与 `tests/contract/team-economy-contract.ps1`
- RunStore changed-run 增量写与 52 周/10,000 run 并发容量证据：`services/control-api/Infrastructure/ExtractionRunStore.cs`、`tests/run-store-capacity/` 与 `tests/integration/run-store-capacity-smoke.ps1`
- 管理台功能懒加载和 bundle 预算：`apps/console-web/src/app/App.tsx`、`apps/console-web/bundle-budget.json`、`apps/console-web/scripts/check-bundle-budget.mjs` 与 `apps/console-web/BUNDLE_BUDGET.md`
- 联邦 v2 caller/target/subject 签名、轮换、吊销及线协议：`services/control-api/Infrastructure/FederationOptions.cs`、`FederationProfiles.cs`、`FederationNodeClient.cs`、`FederationEndpoints.cs`、`tests/federation/`、`tests/integration/federation-smoke.ps1`、`tests/contract/federation-contract.ps1` 与 `docs/runbooks/server-federation-and-compatibility.md`
- 受控世界/经济恢复的停服围栏、跨进程锁、持久恢复日志、双人签名审批、逐文件验证与默认关闸：`tools/world-restore/`、`tests/integration/world-restore-smoke.ps1` 与 `docs/runbooks/save-management.md`；这不替代脱敏真实备份的独立恢复演练
- 兑换区校准的可信采集/复核签名、部署与内容版本固定、共享几何边界、路线/API 证据和离线验证：`tools/zone-calibration/`、`services/control-api/Infrastructure/ExtractionZoneGeometry.cs`、`tests/integration/zone-calibration-smoke.ps1` 与 `docs/runbooks/zone-calibration.md`；这不替代第二点位的真实 Palworld 实测
- 连续周档经济报告的只读权威重算、canonical 归档、稳定伪匿名、受信双人签名复核与篡改拒绝：`tools/weekly-economy-report/`、`services/control-api/Infrastructure/WeeklyEconomyReportArchive.cs`、`tests/weekly-economy-report/`、`tests/integration/weekly-economy-report-smoke.ps1` 与 `docs/runbooks/weekly-economy-report.md`；合成两周不替代连续两个真实周档
- 生产 soak 的进程/数据/日志实例绑定、嵌入阈值、完成请求/p95 门禁和原子证据目录：`tools/soak/`、`services/control-api/Infrastructure/EconomyOperationsEndpoints.cs`、`tests/soak/`、`tests/integration/soak-runner-smoke.ps1` 与 `tests/contract/soak-observability-contract.ps1`；短时 CI profile 不替代正式 24 小时 profile
- 22 项外部门禁的固定 catalog/schema、签名主体信任、组合版本绑定、子证据递归校验与敏感信息扫描：`tools/acceptance-evidence/`、`tests/acceptance-evidence-policy/`、`tests/integration/acceptance-evidence-smoke.ps1` 与 `docs/runbooks/acceptance-evidence.md`；工具通过不等于外部门禁已经取得证据
