  ...
  return (
    <section className="extraction-map-page">
      <div className={`range-status ${status.tone}`} role="status" aria-live="polite">
        ...
        <div>
          <span className="eyebrow">LIVE EXTRACTION STATUS</span>
      ...
      {error ? <div className="inline-tip warning">撤离位置暂时无法更新：{error}</div> : null}
      ...
      <div className="extraction-map-layout">
        <div className="extraction-map-card">
          ...
          <div className="extraction-map-canvas" aria-label="撤离点地图">
            {!backgroundFailed ? (
              <img
                alt="Palworld 主地图与撤离点"
                draggable={false}
            ...
            {mappedZones.map(({ zone, projected, radiusPercent }) => projected?.inBounds ? (
              <button
                aria-label={`查看撤离点 ${zone.displayName || zone.id}`}
              ...
              >
                ...
                <span className="extraction-zone-pin"><i />撤</span>
            ...
            {loading && !data ? <div className="map-data-state"><i /><strong>正在读取撤离点</strong></div> : null}
            {!loading && zones.length === 0 ? <div className="map-data-state"><strong>当前没有开放的撤离点</strong><small>请等待管理员配置行动区域</small></div> : null}
            {!loading && zones.length > 0 && !mappedZones.some((item) => item.projected?.inBounds) ? (
              <div className="map-data-state"><strong>撤离点坐标暂不可绘制</strong><small>仍可在右侧查看服务器返回的信息</small></div>
            ) : null}

            <div className="extraction-map-legend" aria-label="地图图例">
              <span><i className="zone" />撤离区域</span>
        ...
        <aside className="extraction-zone-panel">
          <div className="zone-panel-heading">
            <div><span className="eyebrow">EXTRACTION ZONES</span><h3>撤离点情报</h3></div>
          ...
          {selected ? (
            <>
              ...
              <dl className="zone-facts">
                ...
                <div><dt>距撤离点中心</dt><dd>{formatMapValue(selected.distance, "地图单位")}</dd></div>
              </dl>
              ...
              <div className="zone-list" aria-label="全部撤离点">
                {zones.map((zone) => (
          ...
          ) : <div className="zone-panel-empty">撤离点数据尚未就绪</div>}

          <div className="command-hint">
            <span aria-hidden="true">/</span>
            <div><strong>游戏内快速查询</strong><small>聊天框输入 <code>!撤离</code> 查看最近撤离点与距离。不要使用游戏保留的 <code>/</code> 管理员命令前缀。</small></div>
          </div>
        </aside>
      </div>
    </section>
...
function getRangeStatus(
  ...
  if (enteredZone) return {
    ...
    icon: "✓",
    title: "已进入撤离区域",
    detail: `当前位置位于「${enteredZone.displayName || enteredZone.id}」，可进行战利品扫描与结算。`
  };
  if (!online) return { tone: "unknown", icon: "?", title: "当前位置未知", detail: "角色当前离线，上线后将自动更新与撤离点的距离。" };
  if (loading && zones.length === 0) return { tone: "unknown", icon: "…", title: "正在确认撤离位置", detail: "正在读取角色位置和服务器撤离点配置。" };
  if (error) return { tone: "unknown", icon: "!", title: "当前位置暂不可用", detail: "位置读取失败，不会猜测你是否已进入撤离区域。" };
  if (!positionAvailable) return { tone: "unknown", icon: "?", title: "当前位置暂不可用", detail: statusMessage?.trim() || "服务器尚未返回可靠的位置坐标，不会显示推测结果。" };
  if (zones.some((zone) => zone.inRange === false)) return { tone: "outside", icon: "→", title: "尚未进入撤离区域", detail: "按地图标记前往撤离点；进入有效半径后状态会自动变化。" };
