...
当前源码候选锁定 Palworld `v1.0.1.100619`、Steam build `24181105`、Palworld 专用 UE4SS 提交 `c2ac246` 和精确 PalServer EXE 大小/SHA-256。候选版本为 `0.3.0-dev.39-ro`、Bridge 协议 `1.1`，默认只声明和执行只读探针；两个全新独立、Cargo `--locked` 的受控目录均复现 893,440 字节、SHA-256 `c2dab9f9bfd3c47ac1a244139fb96ce1de6f598c4bce438ebddde96185063b34` 的字节一致 DLL。该 dev39-ro 精确候选尚未在当前 PalServer 受控加载，也未执行固定运行探针套件，Bridge 可用性必须保持 unknown。最近的运行证据属于 888,320 字节、SHA-256 `012f84929448321196734b0bdc1f1b6a899a6f7a0aa87564d99b6e4c40b868aa` 的 dev38-ro：运行时身份校验通过，9 项不依赖在线玩家的 schema/catalog/UI 操作成功，玩家、成长与库存 3 项 live probe 因无人在线拒绝，0 项意外失败；该版本现已 superseded/quarantined，不得把结果转记给 dev39-ro。当前候选仍是 `read-only-candidate-unverified`：还缺 dev39-ro 受控加载、固定套件、受控在线玩家、PalDefender 组合和独立复核，不构成完整 P0-04 验收。C++ 模组必须针对运行时的精确 ABI 重新编译；`CppUserModBase` 的虚函数表会随 UE4SS 提交变化，产物不能跨提交复用。

候选在注册任何 Unreal hook 或创建 Named Pipe 前，会用拒绝并发写入/删除的文件句柄流式计算宿主 PalServer EXE、当前 PalControlNative DLL 和已加载 UE4SS DLL 的 SHA-256/大小，并在结束前复核 file id、长度与修改时间。宿主 EXE 或 UE4SS 任一值与锁文件不一致时直接退出初始化；Native 自身摘要由 hello 上报并与锁比较，Control API 还经 pipe server PID 独立哈希主 EXE，避免普通同名 pipe 进程伪造宿主。`inventory.probe` 只有在当前权威世界的有效 `PalPlayerController.PlayerState` 引用能解析到同一 PlayerUID 时才输出 `ownerOnline=true`；Control API 对该字段缺失或 false 都拒绝报价/结算。即使本机客户端绕过 capability 协商发送写命令，游戏线程 adapter 仍返回 `NATIVE_WRITE_CAPABILITIES_QUARANTINED`。当前构建同时关闭聊天 hook、随机复活 hook 和所有背包/成长/帕鲁/公告写能力。
...
旧 `v1.0.0.100427`/dev36 组合曾验证真实 `hello` 和持续 heartbeat。dev37-ro 随后在当前版本受控加载时暴露出“持久离线库存被误当作 live inventory”的边界缺陷，已被隔离。dev38-ro 完成过 9 项非玩家成功、3 项无人在线拒绝、0 项意外失败的受控只读套件，但现已由 dev39-ro 源码候选取代并保持 superseded/quarantined，禁止重新部署。dev39-ro 必须先完成固定运行套件，再按 `p0-04-current-native-probe` 由受控在线玩家完成玩家、成长和三容器 inventory probe，并经独立复核后，才允许另行评审写能力构建。

## 游戏内撤离点查询

以下实现保留在源码中，但 dev39-ro 只读候选不会安装该聊天 hook。只有后续独立写能力评审明确批准并重新构建时才可能启用。
...
以下实现保留在源码中，但 dev39-ro 只读候选的 CMake 默认值和受控构建脚本均强制关闭该 hook；已淘汰的 dev37-ro 与已 superseded 的 dev38-ro 也不得用于恢复该能力。
...
1. 旧 dev36 与 Control API 的 `hello`/heartbeat 曾在旧版真实 PalServer + UE4SS 进程中验证；dev37-ro 因离线库存边界缺陷已淘汰。dev38-ro 已完成精确身份的受控只读加载和 9 成功/3 无在线玩家拒绝/0 意外失败探针，但现已 superseded/quarantined。当前 dev39-ro 仅完成双独立字节一致构建，尚未实服加载或执行固定套件。
...
5. dev39-ro 的固定套件与 `p0-04-current-native-probe` 通过且由独立复核人批准后，才能另建显式写能力候选；当前 dev39-ro 只读候选、已 superseded 的 dev38-ro 和已淘汰的 dev37-ro 都不可用于持久化扣物验收。
